...
#item family stats
item_city_dw_mean = train[['item_nbr','city','dow','unit_sales']].groupby(['item_nbr','city','dow'])['unit_sales'].mean().to_frame('city_madw').reset_index()
item_city_wk_mean = item_city_dw_mean[['item_nbr','city','city_madw']].groupby(['item_nbr', 'city'])['city_madw'].mean().to_frame('city_mawk').reset_index()
#Days of Week Means
...
ma_is_city = train[['item_nbr','city','unit_sales']].groupby(['item_nbr','city'])['unit_sales'].mean().to_frame('city_mais226')
for i in [112,56,28,14,7,3,1]:
    tmp = train[train.date>lastdate-timedelta(int(i))]
    tmpg = tmp.groupby(['item_nbr','store_nbr'])['unit_sales'].mean().to_frame('mais'+str(i))
    ma_is = ma_is.join(tmpg, how='left')
    tmpg = tmp.groupby(['item_nbr','city'])['unit_sales'].mean().to_frame('city_mais'+str(i))
    ma_is_city = ma_is_city.join(tmpg, how='left')

del tmp,tmpg

ma_is['mais']=ma_is.median(axis=1)
ma_is.reset_index(inplace=True)
ma_is_city['city_mais']=ma_is_city.median(axis=1)
ma_is_city.reset_index(inplace=True)
test = pd.merge(test, ma_is, how='left', on=['item_nbr','store_nbr'])
test = pd.merge(test, ma_wk, how='left', on=['item_nbr','store_nbr'])
test = pd.merge(test, ma_dw, how='left', on=['item_nbr','store_nbr','dow'])
#item family stats
test_fam = pd.merge(test, ma_is_city, how='left', on=['item_nbr','city'])
test_fam = pd.merge(test_fam, item_city_wk_mean, how='left', on=['item_nbr','city'])
test_fam = pd.merge(test_fam, item_city_dw_mean, how='left', on=['item_nbr','city','dow'])
del ma_is, ma_wk, ma_dw, ma_is_city,item_city_wk_mean,item_city_dw_mean
...
test_fam.loc[~pos_idx,'unit_sales'] = test_zeros['city_mais'] * test_zeros['city_madw'] / test_zeros['city_mawk']
...
test_fam[['id','unit_sales']].to_csv('ma8dwof-fixzeros-meanitem-city.csv.gz', index=False, float_format='%.3f', compression='gzip')
