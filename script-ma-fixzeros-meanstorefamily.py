...
#item family stats
item_family_dw_mean = train[['store_nbr','family','dow','unit_sales']].groupby(['store_nbr','family','dow'])['unit_sales'].mean().to_frame('fam_madw').reset_index()
...
test_fam.loc[~pos_idx,'unit_sales'] = test_zeros['fam_mais'] * test_zeros['fam_madw'] / test_zeros['fam_mawk']
...
test_fam[['id','unit_sales']].to_csv('ma8dwof-fixzeros.csv.gz', index=False, float_format='%.3f', compression='gzip')
len(test_fam)
